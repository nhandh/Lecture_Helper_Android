...
public class DisplayCourse extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        lv_days_of_week = (ListView) findViewById(R.id.DisplayCourse_daysOfWeek);
        lv_days_of_week.setChoiceMode(ListView.CHOICE_MODE_MULTIPLE);
        lv_days_of_week.setAdapter
        ...
        if (extras != null){
            ...
            if (value > 0){
                ...
                int start_hour = rs.getInt(rs.getColumnIndex(DBHelper.COURSES_COLUMN_START_HOUR));
                int start_minute = rs.getInt(rs.getColumnIndex(DBHelper.COURSES_COLUMN_START_MINUTE));
                int end_hour = rs.getInt(rs.getColumnIndex(DBHelper.COURSES_COLUMN_END_HOUR));
                int end_minute = rs.getInt(rs.getColumnIndex(DBHelper.COURSES_COLUMN_END_MINUTE));
                ...
                for (int i =0; i<days_of_week.length(); i++){
                    ...
                    if (c == '0'){
                        lv_days_of_week.setItemChecked(i, false);
                    }
                    else{
                        lv_days_of_week.setItemChecked(i, true);
                    }
                }
                lv_days_of_week.setFocusable(false);
                lv_days_of_week.setEnabled(false);
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        switch (item.getItemId()){
            case R.id.DisplayCourse_menu_edit_course:
                ...
                lv_days_of_week.setEnabled(true);
    ...
    public void run(View view){
        ...
        if (extras != null){
            ...
            //String days_of_week = "0000000";
            String days_of_week = "";

            int len = lv_days_of_week.getCount();
            SparseBooleanArray checked = lv_days_of_week.getCheckedItemPositions();
            for (int e = 0; e < len; e++) {
                if (checked.get(e)) {
                    days_of_week += "1";
                }
